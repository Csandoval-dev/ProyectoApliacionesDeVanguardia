...
const createUser = async (userData) => {
    ...
    // AQUÍ ESTÁ EL CAMBIO: Verificar si ya está hasheada
    let hashedPassword;
    
    // Si la contraseña parece ya estar hasheada (bcrypt hashes empiezan con $2a$, $2b$, etc.)
    if (contrasena.startsWith('$2a$') || contrasena.startsWith('$2b$') || contrasena.startsWith('$2y$')) {
        hashedPassword = contrasena; // Ya está hasheada, no la tocamos
        console.log('Contraseña ya está hasheada, no se modifica');
    } else {
        // Hashear la contraseña solo si no está hasheada
        hashedPassword = await bcrypt.hash(contrasena, 10);
        console.log('Contraseña hasheada por primera vez');
    }
...
const checkPasswordHash = async (plainPassword, hashedPassword) => {
    try {
        const isValid = await bcrypt.compare(plainPassword, hashedPassword);
        console.log('Password check:', { plainPassword, hashedPassword, isValid });
        return isValid;
    } catch (error) {
...
module.exports = { 
    ...
    getUsersByClinic,
    updateUserClinic,
    checkPasswordHash // Exportamos para debugging
};